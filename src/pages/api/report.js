...
function computeRisks(answers, stateSalesRows, empStates = []) {
  ...
  const risks = [];
  const add = (category, title, text, severity) => risks.push({ category, title, text, severity });

  // ── Federal Tax ──────────────────────────────────────────────────────────
  if (a.prior_reorg === "yes" && a.prior_diligence === "no")
    add("Federal Tax", "Prior Acquisition Without Tax Diligence",
      "Potential risk that tax liabilities from prior reorganizations or acquisitions were not identified prior to closing. Recommend reviewing prior transaction documents and any available tax diligence from that transaction.", "moderate");

  // ERC — single consolidated finding
  if (a.erc_claimed === "yes") {
    if (a.erc_q3_2021 === "yes") {
      add("Federal Tax", "ERC Claim — Extended Statute Open",
        "Potential risk of IRS disallowance of Employee Retention Credits claimed. The statute of limitations for Q3 2021 credits runs through April 15, 2027, or 2 years from the date the credit was received, whichever is later. Recommend reviewing eligibility documentation supporting the claim.", "moderate");
    } else if (a.erc_q3_2021 === "no" && a.erc_received_2yr === "yes") {
      add("Federal Tax", "ERC Claim Within Assessment Window",
        "Potential risk of IRS disallowance of a portion or all of Employee Retention Credits claimed. Credits received within the past 2 years remain within the assessment window. Recommend reviewing eligibility documentation supporting the claim.", "moderate");
    } else if (a.erc_q3_2021 === "no" && a.erc_received_2yr === "no") {
      add("Federal Tax", "ERC Claim — Statute May Have Lapsed",
        "Low risk. Employee Retention Credits were claimed for quarters where the statute of limitations may have lapsed. Recommend confirming filing and receipt dates to verify the assessment window has closed.", "low");
    }
  }

  if (a.tax_exam === "yes" && a.tax_exam_resolved === "no")
    add("Federal Tax", "Open Tax Examination Unresolved",
      "Elevated risk. The Company is subject to an open or unresolved tax examination. The outcome may result in additional tax, penalties, and interest. Recommend obtaining all examination correspondence, understanding the issues under review, and quantifying any proposed adjustments prior to closing.", "high");
  if (a.tax_exam === "yes" && a.tax_exam_resolved === "yes")
    add("Federal Tax", "Prior Tax Examination Resolved",
      "Low risk. The Company has been subject to a tax examination that has been resolved. Recommend confirming all assessments have been paid in full, that amended returns were filed where required, and that any procedural deficiencies identified during the audit have been addressed.", "low");

  if (a.related_party === "yes" && a.related_party_fmv === "no")
    add("Federal Tax", "Related Party Transactions Not at FMV",
      "Elevated risk that related party transactions not conducted at fair market value may be recharacterized by the IRS, resulting in additional taxable income, denied deductions, or other adjustments. Recommend obtaining documentation supporting the pricing of all related party transactions.", "high");
  if (a.related_party === "yes" && a.related_party_fmv === "yes")
    add("Federal Tax", "Related Party Transactions at FMV",
      "Low risk. The Company has related party transactions conducted at fair market value. Recommend confirming that contemporaneous documentation exists to support FMV pricing in the event of an audit.", "low");
  ...
  if (a.sales_tax_nexus === "yes") {
    ...
    add("Sales & Use Tax", "Sales Tax Economic Nexus", text, "moderate");
  }

  if (a.exemption_certs === "no")
    add("Sales & Use Tax", "Missing Exemption Certificates",
      "Risk that undocumented exempt sales will be presumed taxable under audit and assessed against the Company with penalties and interest. Recommend implementing a process to collect and periodically refresh exemption certificates from all customers claiming exemption.", "moderate");

  if (a.use_tax_review === "no")
    add("Sales & Use Tax", "Use Tax Self-Assessment Gap",
  ...
  if (a.contractor_usage === "yes" && a.contractor_classification === "no")
    add("Employment Tax", "Contractor Misclassification Risk",
      "Risk of employment tax exposure on independent contractor compensation. Without a documented classification process, contractors performing employee-equivalent functions may be reclassified under audit, resulting in back payroll taxes, penalties, and interest.", "moderate");
  ...
  if (a.property_tax === "no")
    add("Property Tax", "Personal Property Tax Filing Gap",
      "Risk of property tax liability in states where the Company owns or leases real or personal property but has not filed property tax returns. Note that not all states impose personal property tax — applicability varies by state. Recommend reviewing the Company's property locations and confirming filing obligations in each state.", "low");

  // ── Unclaimed Property ───────────────────────────────────────────────────
  if (a.unclaimed_property === "no")
    add("Unclaimed Property", "Unclaimed Property Process Absent",
      "Recommendation: The Company does not appear to have formal processes in place to identify and remit unclaimed property. While this is not a confirmed liability, uncashed checks and unredeemed customer credits may be subject to state escheatment laws after a dormancy period of typically 3 to 5 years. Recommend implementing a process to identify, review, and remit unclaimed property to applicable states on an annual basis.", "recommendation");

  // ── Equity deal risks ─────────────────────────────────────────────────────
  // S Corporation
  if (a.scorp_single_class === "no")
    add("Federal Tax", "S Corp Second Class of Stock",
      "Risk that S corporation eligibility may be invalid due to a second class of stock. Potential C corporation reclassification and corporate-level tax exposure.", "high");
  if (a.scorp_shareholder_count === "yes")
    add("Federal Tax", "S Corp Shareholder Limit Exceeded",
      "Risk that S corporation eligibility may be invalid due to exceeding the 100 shareholder limit.", "high");
  if (a.scorp_eligible_shareholders === "yes")
    add("Federal Tax", "Ineligible S Corp Shareholder",
      "Risk that S corporation eligibility may be invalid due to a non-eligible shareholder. Recommend obtaining shareholder history and confirming IRS acceptance.", "high");
  if (a.scorp_election_docs === "no")
    add("Federal Tax", "S Corp Election Not Documented",
      "Risk that S corporation status may not be valid without documentation. Recommend obtaining IRS acceptance letter.", "high");
  if (a.scorp_big_assets === "yes")
    add("Federal Tax", "Built-In Gains Tax Exposure",
      "Risk of built-in gains tax on appreciated assets at the highest corporate rate. Quantify appreciated assets at the time of conversion.", "high");

  // C Corporation
  if (a.ccorp_ownership_change === "yes")
    add("Federal Tax", "Section 382 NOL Limitation Risk",
      "Risk of Section 382 limitation on NOL and credit carryforwards due to ownership change exceeding 50%.", "moderate");
  if (a.ccorp_nol === "yes") {
    ...
    add("Federal Tax", "NOL Carryforward Limitation", nolText, "moderate");
  }
  if (a.ccorp_credits === "yes")
    add("Federal Tax", "Tax Credit Carryforward Limitation",
      "Risk that tax credit carryforwards are subject to Section 383 limitations following an ownership change.", "low");

  // Partnership / LLC
  if (a.pship_rep === "no")
    add("Federal Tax", "No Partnership Representative Designated",
      "Risk that the IRS will appoint a Partnership Representative, causing the partnership to lose control over audit proceedings under the BBA regime.", "low");
  if (a.pship_pushout === "no")
    add("Federal Tax", "Push-Out Election Provision Absent",
      "Risk that audit adjustments will be assessed at the partnership level at the highest tax rate rather than flowing through to individual partners.", "moderate");
  if (a.pship_aar === "yes")
    add("Federal Tax", "Open Administrative Adjustment Requests",
      "Risk of open adjustments from amended returns or Administrative Adjustment Requests that may flow through to partners post-closing. Recommend obtaining copies of all filed AARs and amended returns, identifying the nature and amount of any adjustments, and determining whether any amounts remain open or subject to partner-level flow-through after closing.", "moderate");
  if (a.pship_bba_alloc === "no")
    add("Federal Tax", "BBA Audit Liability Unallocated",
      "Risk that the buyer may bear the economic burden of pre-closing audit adjustments with no contractual protection.", "moderate");

  // Common equity questions
  if (a.eq_open_years === "yes")
    add("Federal Tax", "Open Federal Tax Years",
      "As is typical in most transactions, risk of exposure on open tax years where federal or state authorities may still assess additional tax. Recommend tax representation and indemnification provisions in the purchase agreement. For larger transactions, representations and warranties insurance may provide additional protection.", "low");
  if (a.eq_notices === "yes")
    add("Federal Tax", "Outstanding Tax Notices Received",
      "Risk of unresolved tax positions indicated by outstanding notices. Obtain copies and assess exposure.", "moderate");
  if (a.eq_utp === "yes")
    add("Federal Tax", "Uncertain Tax Positions on Balance Sheet",
      "Risk of uncertain tax positions on the balance sheet. Quantify ASC 740 reserves and assess likelihood of settlement.", "moderate");
